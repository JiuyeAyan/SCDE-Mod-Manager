...
namespace System.IO
{
    ...
    internal static class PathInternalNetCore
    {
        ...
        internal unsafe static int EqualStartingCharacterCount(string first, string second, bool ignoreCase)
        {
            ...
            // Pin both strings manually using GCHandle, avoiding OffsetToStringData
            GCHandle h1 = GCHandle.Alloc(first, GCHandleType.Pinned);
            GCHandle h2 = GCHandle.Alloc(second, GCHandleType.Pinned);

            try
            {
                // Pointer to first character of each string
                char* p1 = (char*)h1.AddrOfPinnedObject().ToPointer();
                char* p2 = (char*)h2.AddrOfPinnedObject().ToPointer();
                ...
                if (ignoreCase)
                {
                    for (int i = 0; i < len; i++)
                    {
                        char c1 = char.ToUpperInvariant(p1[i]);
                        char c2 = char.ToUpperInvariant(p2[i]);
                        if (c1 != c2) break;
                        count++;
                    }
                }
