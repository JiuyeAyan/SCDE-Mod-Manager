...
/// <summary>
/// Flags used for message sending. As taken from Steamworks docs.
/// https://partner.steamgames.com/doc/api/steamnetworkingtypes?l=english
/// </summary>
[Flags]
internal enum SteamNetworkingSend : int
{
    /// <summary>
    ...
    /// By default, Nagle's algorithm is applied to all outbound messages. This means that the message will NOT be sent immediately, in case further messages are sent soon after you send this, which can be grouped together. 
    /// Any time there is enough buffered data to fill a packet, the packets will be pushed out immediately, but partially-full packets not be sent until the Nagle timer expires. 
    /// See ISteamNetworkingSockets::FlushMessagesOnConnection, ISteamNetworkingMessages::FlushMessagesToUser
    /// NOTE: Don't just send every message without Nagle because you want packets to "get there quicker". Make sure you understand the problem that Nagle is solving before disabling it. 
    /// If you are sending small messages, often many at the same time, then it is very likely that it will be more efficient to leave Nagle enabled. 
    /// A typical proper use of this flag is when you are sending what you know will be the last message sent for a while (e.g. the last in the server simulation tick to a particular client), and you use this flag to flush all messages.
    /// </summary>
    NoNagle = 1,
    ...
    /// This is equivalent to using steamnetworkingtypes::k_ESteamNetworkingSend_Unreliable and then immediately flushing the messages using ISteamNetworkingSockets::FlushMessagesOnConnection or ISteamNetworkingMessages::FlushMessagesToUser.
    /// (But using this flag is more efficient since you only make one API call.)
    /// </summary>
    k_nSteamNetworkingSend_UnreliableNoNagle = Unreliable | NoNagle,
    ...
    /// </summary>
    k_nSteamNetworkingSend_UnreliableNoDelay = Unreliable | Nodelay | NoNagle,
    ...
    /// 
    /// If data is not ready to be sent (due to rate limiting
